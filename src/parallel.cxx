...
isl_union_map * pth_compute_dependencies(pth_scop * scop) { 
  ...
  // modify for new pet.h
  isl_union_map * reads  = pet_scop_collect_may_reads(scop->scop);
  isl_union_map * writes = pet_scop_collect_may_writes(scop->scop);
...
int recursive_band_list(isl_printer * printer, isl_band_list * list, struct index_list * indent) {
  ...
  for (int i = 0 ; i < band_count ; i++) {
    ...
    for (int j = 0 ; j < band_member_count ; j++ ) {
      ...
      // new func applied
      int is_zero_distance = isl_band_member_is_coincident(band, j);
